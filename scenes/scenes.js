...
ageSceneRenter.enter((ctx) => {
  ctx.session.lastInteraction = Date.now();
  ctx.reply('Будь ласка, введіть свій вік:');
});
...
budgetSceneRenter.enter((ctx) => {
  ctx.session.lastInteraction = Date.now();
  ctx.reply('Вкажіть бажану ціну за житло (грн):');
});
...
roomSceneRenter.enter((ctx) => {
  ctx.session.lastInteraction = Date.now();
  ctx.reply('Кількість кімнат:');
});
...
floorSceneRenter.enter((ctx) => {
  ctx.session.lastInteraction = Date.now();
  ctx.reply('Бажаний поверх:');
});
...
animalsSceneRenter.enter((ctx) => {
  ctx.session.lastInteraction = Date.now();
  ctx.reply('Чи є у вас тварини, якщо так, то скільки та які:');
});
...
childrenSceneRenter.enter((ctx) => {
  ctx.session.lastInteraction = Date.now();
  ctx.reply('Скільки осіб планує проживати, діти:');
});
...
conditionSceneRenter.enter((ctx) => {
  ctx.session.lastInteraction = Date.now();
  ctx.reply('Напишіть, які умови ви очікуєте (ремонт, наявність меблів та техніки тощо):');
});
...
contactSceneRenter.enter((ctx) => {
  ctx.session.lastInteraction = Date.now();
  ctx.reply('Будь ласка, введіть номер для зворонього зв\'язку (формат: 050 000 1111):');
});
